...
export let widthRatio = canvas.width/1920;
export let heightRatio = canvas.height/1080;

export function resScale (useHeightRatio) {
    if (!useHeightRatio) {
        return widthRatio;
    } else {
        return heightRatio;
...
export let canvasWidthBeforeResize;
export let canvasHeightBeforeResize;

window.addEventListener('resize', (event) => {
    canvasWidthBeforeResize = canvas.width;
    canvasHeightBeforeResize = canvas.height;
    ...
    widthRatio = canvas.width/1920;
    heightRatio = canvas.height/1080;

    initializeButtons();
    resizePlayerCustomizationInputs();
    resScalePlayers(canvasWidthBeforeResize, canvasHeightBeforeResize);
});
...
export let sandTexture = new Image(); sandTexture.src = './modules/scenes/Ballf With Your Friends/sandTexture.png';
export let waterTexture = new Image(); waterTexture.src = './modules/scenes/Row Race/waterTexture.jpg';
export let waterTexture2 = new Image(); waterTexture2.src = './modules/scenes/Ballf With Your Friends/waterTexture2.png';
...
//players import
import { initializePlayers, resScalePlayers } from './modules/players.js';
initializePlayers();
...
import { initializeCreditsScreenButtons, creditsButtonsArray } from './modules/buttons.js';
...
*/
import { initializeButtons, creditsButtonsArray, buttonPlayer, buttonPlayerCustomizationOption } from './modules/buttons.js';
initializeButtons();
spaceEvenly(creditsButtonsArray, false, 100);
spaceEvenly(buttonPlayerCustomizationOption, false);

import { initializePlayerCustomizationInputs, resizePlayerCustomizationInputs } from './modules/inputs.js';
initializePlayerCustomizationInputs();


import { spaceEvenly } from './modules/spaceEvenly.js';
import { gamepadClick, gamepadMenuControls } from './modules/listeners/gamepadMenuControls.js';
import { games } from './modules/scenes/gameSelect.js';
