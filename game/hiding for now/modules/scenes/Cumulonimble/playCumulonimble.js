...
function loseCondition () {
    for (let i = 0; i < playersLeft.length; i++) {
        if (playersLeft[i].y > height) {
            ...
            if (!fourthPlace && player.length >= 4) { fourthPlace = playersLeft.splice(i, 1) }
            else if (!thirdPlace && player.length >= 3) { thirdPlace = playersLeft.splice(i, 1) }
            else if (!secondPlace) { secondPlace = playersLeft.splice(i, 1) }
        }
    }
