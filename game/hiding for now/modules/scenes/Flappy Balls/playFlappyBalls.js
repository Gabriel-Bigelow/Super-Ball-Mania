...
function loseHandler (pNum) {
    ...
    if (playersLeft.length <= 4) {
        if (!fourthPlace && player.length >= 4) { fourthPlace = playersLeft.splice(playersLeft.indexOf(player[pNum]), 1); scoreWhenLastDied = score; }
        else if (!thirdPlace && player.length >= 3) { thirdPlace = playersLeft.splice(playersLeft.indexOf(player[pNum]), 1); scoreWhenLastDied = score; }
        else if (!secondPlace) { secondPlace = playersLeft.splice(playersLeft.indexOf(player[pNum]), 1); scoreWhenLastDied = score; }    
    }
