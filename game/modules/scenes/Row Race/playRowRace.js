import { birdAnimation, buttonPic, c, centerX, centerY, height, rowRaceTrack, waterTexture, width } from "../../../Main.js";
...
function checkpointPassedCheck() {
    for (let i = 0; i < player.length; i++){
        ...
        if (player[i].rrp.checkpoint[2] === 4) { 
            player[i].rrp.checkpoint[2] = `${raceTime} seconds`;
...
function leaderboard () {
    ...
    for (let i = -player.length/2; i < player.length/2; i++){
        ...
        if (typeof player[j].rrp.checkpoint[2] === 'number') { c.fillText(`${player[j].name}: Lap ${player[j].rrp.checkpoint[2]}/3`, width*0.31, height*0.435 + 30*i); } 
        else { c.fillText(`${player[j].name}: ${player[j].rrp.checkpoint[2]}`, width*0.31, height*0.435 + 30*i); }
        finishRace(j);
        j += 1;
    }
}
function finishRace (pNum) {
    for (let i = 0; i < player.length; i++) {
        if (player[pNum].rrp.checkpoint[2][player[pNum].rrp.checkpoint[2].length-1] === 's') {
