import { c, width, height, centerX, centerY, resScale } from '../Main.js';
...
export class Input {
    ...
    draw () {
        ...
        c.font = `${this.fontSize * resScale()*1.2}px Oswald`;
        c.fillText(this.text, centerX, this.y-this.inputHeight/2 + this.fontSize*3/2);
        c.font = `${this.fontSize * resScale()}px Oswald`;
    ...
    inputColorSlider () {
        ...
        for (let i = 0; i < incrementButton.length; i++){
            let j = i % 3;
            incrementButton[i].fontSize = 40 * resScale();
        ...
        //console.log(this.colorGrabbed[0]);
        c.fillStyle = 'rgb(0, 0, 0)';
        ...
        //sliders
        let redSliderX = Math.floor(this.x-width/5+this.colorGrabbed[0]*3*resScale());
        let greenSliderX = Math.floor(this.x-width/5+this.colorGrabbed[1]*3*resScale());
        let blueSliderX = Math.floor(this.x-width/5+this.colorGrabbed[2]*3*resScale());
        let sliderWidth = 20*resScale();
        let sliderHeight = 40*resScale();
        ...
        if (mousedown && mouseX > this.x-width/5-sliderWidth/2 && mouseX < this.x+width/5+sliderWidth/2 && mouseY > this.y-this.inputHeight/6 - sliderHeight/2 && mouseY < this.y-this.inputHeight/6 + sliderHeight/2){
            this.colorGrabbed[0] = (mouseX+width/5-this.x)/(3*resScale())
        }
        if (mousedown && mouseX > this.x-width/5-sliderWidth/2 && mouseX < this.x+width/5+sliderWidth/2 && mouseY > this.y+this.inputHeight/12 - sliderHeight/2 && mouseY < this.y + this.inputHeight/12 + sliderHeight/2){
            this.colorGrabbed[1] = (mouseX +width/5-this.x)/(3*resScale())
        }
        if (mousedown && mouseX > this.x-width/5-sliderWidth/2 && mouseX < this.x+width/5+sliderWidth/2 && mouseY > this.y+this.inputHeight/3 - sliderHeight/2 && mouseY < this.y + this.inputHeight/3 + sliderHeight/2){
            this.colorGrabbed[2] = (mouseX +width/5-this.x)/(3*resScale())
        }
        //gamepad sliders
        if (mousedown && gamepadCursorX > this.x-width/5-sliderWidth/2 && gamepadCursorX < this.x+width/5+sliderWidth/2 && gamepadCursorY > this.y-this.inputHeight/6 - sliderHeight/2 && gamepadCursorY < this.y-this.inputHeight/6 + sliderHeight/2){
            this.colorGrabbed[0] = (gamepadCursorX +width/5-this.x)/(3*resScale())
        }
        if (mousedown && gamepadCursorX > this.x-width/5-sliderWidth/2 && gamepadCursorX < this.x+width/5+sliderWidth/2 && gamepadCursorY > this.y+this.inputHeight/12 - sliderHeight/2 && gamepadCursorY < this.y + this.inputHeight/12 + sliderHeight/2){
            this.colorGrabbed[1] = (gamepadCursorX +width/5-this.x)/(3*resScale())
        }
        if (mousedown && gamepadCursorX > this.x-width/5-sliderWidth/2 && gamepadCursorX < this.x+width/5+sliderWidth/2 && gamepadCursorY > this.y+this.inputHeight/3 - sliderHeight/2 && gamepadCursorY < this.y + this.inputHeight/3 + sliderHeight/2){
            this.colorGrabbed[2] = (gamepadCursorX +width/5-this.x)/(3*resScale())
        }

        //preview
        c.fillText('Preview', this.x + this.inputWidth*4/9, this.y-player[selectedPlayer].size*2/3*resScale(true));
        ellipse(this.x + this.inputWidth*4/9, this.y, player[selectedPlayer].size*resScale()+2*resScale(), `rgb(0, 0, 0)`);
        ellipse(this.x + this.inputWidth*4/9, this.y, player[selectedPlayer].size*resScale(), `rgb(${inputColor.colorGrabbed[0]}, ${inputColor.colorGrabbed[1]}, ${inputColor.colorGrabbed[2]})`);
    ...
    inputControls () {
        ...
        let buttonBack = new Button(this.x - this.inputWidth/2+60*resScale(), this.y-this.inputHeight/2+60*resScale(true), 100*resScale(), 100*resScale(true), 'Back');

        let buttonLeft = new Button (this.x - 300*resScale(), buttonYAlign, 200*resScale(), 100*resScale(), 'LEFT');
        let buttonUp = new Button (this.x, buttonYAlign, 200*resScale(), 100*resScale(), 'UP');
        let buttonRight = new Button (this.x + 300*resScale(), buttonYAlign, 200*resScale(), 100*resScale(), 'RIGHT');
        let buttonDown = new Button (this.x - 300*resScale(), buttonYAlign + 100*resScale(true), 200*resScale(), 100*resScale(), 'DOWN');
        let buttonSprint = new Button (this.x + 300*resScale(), buttonYAlign + 100*resScale(true), 200*resScale(), 100*resScale(), 'SPRINT');
...
export function initializePlayerCustomizationInputs () {
    ...
    allInputs = [inputName, inputColor, inputControls];
}

export function resizePlayerCustomizationInputs () {
    allInputs.forEach((element) => {
        if (element === inputName) { element.inputWidth = width*2/5; } else { element.inputWidth = width*4/5; };
        element.inputHeight = height/4;
        element.x = centerX;
        element.y = centerY+height/5;
    });
}