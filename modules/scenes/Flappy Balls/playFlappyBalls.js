import { c, centerX, centerY, flappyBallsBackground, height, heightRatio, pipeBottom, pipeTop, resScale, scene, width, widthRatio } from "../../../Main.js";
import { dImage, ellipse, image } from "../../drawShapes.js";
...
function startGame () {
    if (!gameStarted) {
        ...
        pipeGap = 250 * resScale(true)
        scrollSpeed = 3 * resScale();

        for (let i = 0; i < player.length; i++){
            player[i].lose = false;
            player[i].x = 200 + player[i].size*i*1.5
            player[i].y = height*5/6
...
function renderBackground() {
    for (let i = 0; i < 3; i++) {
        c.drawImage(flappyBallsBackground, imageX + 1920*i, -1080 +height)
    }
    imageX -= scrollSpeed;
    if (imageX < -1920) { imageX %= 2; }
}
...
function jump (pNum) {
    if (jumpButtonHeld[pNum] === 1) {
        gravityOnPlayer[pNum] = -10 * resScale(true)
        jumpCount[pNum]++;
...
function gravity (pNum) {
    ...
    gravityOnPlayer[pNum] += 0.5 * resScale(true);
    if (!player[pNum].lose) {
...
function createPipe () {
    pipe.push( {
        ...
        pipeWidth: pipeTop.naturalWidth * resScale(),
        pipeHeight: pipeTop.naturalHeight * resScale(),
        gap: Math.max(pipeGap, 180*resScale())
    } )
}
...
function renderPipes () {
    ...
    for (let i = 0; i < pipe.length; i++) {
        //c.drawImage(pipe[i].pic, pipe[i].x, pipe[i].y); commented out is old code before resScale() implementation
        dImage(pipe[i].pic, pipe[i].x, pipe[i].y, widthRatio, resScale(true));
        wall(pipe[i].x, 0, pipeTop.naturalWidth*resScale(), pipe[i].y + pipeTop.naturalHeight*resScale(true));
        dImage(pipe[i].pic, pipe[i].x, pipe[i].y+pipeTop.naturalHeight*resScale(true)+pipe[i].gap, widthRatio, resScale(true));
        //c.drawImage(pipe[i].pic, pipe[i].x, pipe[i].y+pipeTop.naturalHeight+pipe[i].gap);
        wall(pipe[i].x, pipe[i].y+pipeTop.naturalHeight*resScale()+pipe[i].gap, pipeTop.naturalWidth*resScale(true), height);
        for (let j = 1; j < 7; j++) {
            dImage(pipeBottom, pipe[i].x, pipe[i].y-pipeBottom.naturalHeight*resScale(true)*j, widthRatio, resScale(true))
            dImage(pipeBottom, pipe[i].x, pipe[i].y+pipeTop.naturalHeight*resScale(true)*2+pipeBottom.naturalHeight*(j-1)*resScale(true) + pipe[i].gap, resScale(), resScale(true));
            //dImage(pipeBottom, pipe[i].x, pipe[i].y+pipeTop.naturalHeight*resScale(true)+pipe[i].gap);
        }
...
function renderScore () {
    ...
    c.beginPath();
    c.rect(centerX - 45*resScale(), height*0.04*resScale(true), 90*resScale(), 90*resScale(true))
    c.closePath();
    c.stroke();
}
...
export function resetGameFlappyBalls() {
    bgm = new Audio('./modules/scenes/Cumulonimble/sounds/bgm.mp3')
    imageX = 0;
    score = 0;
    pipe = [];
    gameStarted = false;
