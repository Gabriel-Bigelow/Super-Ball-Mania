import { c, centerX, centerY, cloudSprite, cumulonimbleBackground, height, resScale, width } from "../../../Main.js";
import { dImage } from "../../drawShapes.js";
...
function startGame () {
    if (!gameStarted) {
        ...
        for (let i = 0; i < player.length; i++){
            ...
            player[i].x = cloud[0].x + (cloud[0].pic.naturalWidth*resScale())/(player.length+1)*(i+1)
        ...
        timeBetweenSpawn = Math.min(270*resScale(true), 270)
        c.textAlign = 'center';
        c.font = '40px oswald';
...
function renderClouds () {
    ...
    for (let i = 0; i < cloud.length; i++) {
        ...
        dImage(cloud[i].pic, cloud[i].x, cloud[i].y, resScale(), resScale(true))
    ...
    cloudSpeed += 0.001*resScale(true)
    frame++;
}
...
export function resetGameCumulonimble () {
