import { buttonPic, c, centerX, centerY, concreteTexture, conveyorBelt, height, lavaTexture, resScale, width } from "../../../Main.js";
...
function startGame () {
    if (gameStart === false) {
        ...
        for (let i = 0; i < player.length; i++){
            player[i].x = centerX-(1470*resScale())/2 + (163+1/3)*2*resScale()
...
function renderPlayers () {
    for (let i = 0; i < player.length; i++){
        c.save();
        c.beginPath()
        c.rect(centerX-(1420*resScale())/2, player[i].y+(7-50)*resScale(true), 1450*resScale(), 100*resScale(true));
        c.closePath();
        c.clip();
        for (let j = -1; j < 2; j++){
            image(conveyorBelt, player[i].x+width/4 + (1470*resScale())*j, player[i].y+7*resScale(true));
        }
        c.restore();
        player[i].ssp.draw();
        ...
        if (!simonSpeaking && player[i].ssp.inputArray.length < commandArray.length && player[i].x >= centerX-(1470*resScale())/2 + (163+1/3)*1*resScale() - 20*resScale() && player[i].x < centerX-(1500*resScale())/2 + (163+1/3)*9*resScale()){
...
function renderArena () {
    for (let i = 0; i < 4; i++){
        for (let j = 0; j < 4; j++){
            c.drawImage(concreteTexture, 640*resScale()*i, 430*resScale(true)*j);
        }
    }
    c.fillStyle = 'rgb(0, 0, 0)';
    c.fillRect(0, 0, 275*resScale(), height);

    c.fillStyle = 'rgb(255, 255, 255)';
    c.fillRect(centerX+(1420*resScale())/2, 0, 400*resScale(), height);
    c.fillStyle = 'rgb(0, 0, 0)';
    for (let i = 0; i < 12; i++){
        for (let j = 0; j < 5; j++){
            let k = i;
            if (j % 2 !== 0){c.fillRect(centerX+(1420*resScale())/2 + 50*resScale()*j, 0 + 100*resScale(true) * i + 50*resScale(true), 50*resScale(), 50*resScale(true)); }
            else { c.fillRect(centerX+(1420*resScale())/2 + 50*resScale()*j, 0 + 100*resScale(true) * k, 50*resScale(), 50*resScale(true)); }
        }
    }
}
...
function renderSimonText () {c
    ...
    if (sayingWhichButtons) {
        for (let i = 0; i < commandArrayCopy.length; i++) {
            image(buttonPic[commandArrayCopy[i]], centerX + i*82*resScale() - (commandArrayCopy.length-1)/2*82*resScale(), centerY, 1*resScale(), 1*resScale(true))
        }
    }
...
function checkPlayerInput () {
    if (followSimonSays){
        finishedPlayers = 0;
        for (let i = 0; i < player.length; i++){
            if (player[i].ssp.inputArray.length === commandArrayCopy2.length || player[i].lose || player[i].x > centerX-(1500*resScale())/2 + (163+1/3)*9*resScale()) {
                if (!firstToComplete){
                    let matchedArray = [];
...
function roundWinHandler () {
    ...
    for (let i = 0; i < player.length; i++){
        setTimeout(() => {
            ...
            let playerInBounds = () => { if (player[i].x > centerX - (1400*resScale())/2 && player[i].x < centerX + (1400*resScale())/2) { return true; } else { return false; }}
            if (firstToComplete === player[i]) { player[i].ssp.xPosition += (163 + 1/3)*resScale(); firstToComplete = undefined; }
            if (matching && followSimonSays && playerInBounds()) { player[i].ssp.xPosition += (163 + 1/3)*resScale(); moveForward.play(); conveyorBeltNoise.play(); numOfCommands += 0.5 } 
            if (!matching && followSimonSays && playerInBounds()) { player[i].ssp.xPosition -= (163 + 1/3)*resScale(); moveBack.play(); conveyorBeltNoise.play(); numOfCommands -= 1;}
            if (player[i].ssp.inputArray.length === 0 && !followSimonSays && playerInBounds()) { player[i].ssp.xPosition += (163 + 1/3)*resScale(); moveForward.play(); conveyorBeltNoise.play(); }
            if (player[i].ssp.inputArray.length > 0 && !followSimonSays && playerInBounds()) { player[i].ssp.xPosition -= (163 + 1/3)*resScale(); moveBack.play(); conveyorBeltNoise.play(); }
...
function movePlayer () {
    for (let i = 0; i < player.length; i++){
        ...
        if (Math.floor(player[i].x) === Math.floor(centerX-(1470*resScale())/2 + (163+1/3)*2*resScale() + 163 * 7 * resScale())){
            placePlayerWin(i);
        }
        if (player[i].x > player[i].ssp.xPosition){
            player[i].x--
        }
        if (Math.floor(player[i].x) === Math.floor(centerX-(1470*resScale())/2 + (163+1/3)*resScale()*2 - 163 * 2*resScale())){
            placePlayerLose(i);
        }
        if (player[i].x < centerX - (1400*resScale())/2){
            player[i].lose = true;
...
export function resetGameSimonSays () {
    bgm = new Audio('./modules/scenes/Simon Says/sounds/elevatorMusic.mp3')
    gameStart = false;
    endGame = false;
    ...
    firstToComplete = undefined;

    whoSaysText = undefined;
    doOrDontText;
    sayingWho = true;
    sayingDoWhat = false;
    sayingWhichButtons = false;
    ...
    numOfCommands = undefined
    commandArray = [];

    whoSays = undefined;
    doOrDont = undefined;
    followSimonSays = undefined;
