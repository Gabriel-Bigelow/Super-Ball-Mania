import { c, centerX, centerY, changeScene, resScale} from '../../Main.js';
...
export function scenePlayerSelect() {
    ...
    function removePlayer() {
        let pNum = player.length-1;
        if (player.length > 2){
            inputControls.keysAssignedTo[player[pNum].left] = { player: pNum, color: player[pNum].clr };
            inputControls.keysAssignedTo[player[pNum].up] = { player: pNum, color: player[pNum].clr };
            inputControls.keysAssignedTo[player[pNum].right] = { player: pNum, color: player[pNum].clr };
            inputControls.keysAssignedTo[player[pNum].down] = { player: pNum, color: player[pNum].clr };
            inputControls.keysAssignedTo[player[pNum].sprint] = { player: pNum, color: player[pNum].clr };
            player.pop();
            buttonPlayer.pop();
        }
