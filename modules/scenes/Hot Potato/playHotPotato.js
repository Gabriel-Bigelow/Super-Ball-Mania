import { brickTexture, c, centerX, centerY, explosionAnimation, height, resScale, smokeAnimation, sparkAnimation, width } from "../../../Main.js";
...
function renderTimer () {
    ...
    c.stroke();
    c.fillText(second, centerX, Math.max(height*0.05*resScale(true), 55*resScale(true)));
    c.closePath();
}
...
function explodeBomb () {
    ...
    if (second === 0){
        ...
        smoke = [];
        spark = [];
        if (!fourthPlace && playersLeft.length === 4) { fourthPlace = playersLeft.splice(playersLeft.indexOf(player[it]), 1) }
        else if (!thirdPlace && playersLeft.length === 3) { thirdPlace = playersLeft.splice(playersLeft.indexOf(player[it]), 1) }
        else if (!secondPlace && playersLeft.length === 2) { secondPlace = playersLeft.splice(playersLeft.indexOf(player[it]), 1) }
        else { playersLeft.splice(playersLeft.indexOf(playersLeft[it]), 1); }
...
export function resetGameHotPotato () {
    bgm = new Audio('./modules/scenes/Hot Potato/sounds/bgm.mp3')
    gameStarted = false;
    endGame = false;
