import { c, width, height, centerX, centerY, boatSprite, buttonPic, scene, resScale, canvas} from '../Main.js';
...
import { sceneTitle } from './scenes/titleScreen.js';
import { forAllPlayers } from './player functions/playerFunctions.js';
import { games } from './scenes/gameSelect.js';
...
export class Player {
    constructor(clr) {
        ...
        this.size = scaledPlayerSize*resScale();
        this.clr = clr;
        this.name;
        this.pid;
        this.skin;
    ...
    north (axes) {
        if (!axes){ axes = -1 }
        if (this.ySpeed >= this.topSpeed*resScale(true)*axes + pAccelRate){
    ...
    west (axes) {
        if (!axes){ axes = -1 }
        if (this.xSpeed >= this.topSpeed*resScale()*axes + pAccelRate){
...
export let scaledPlayerSize = 100;

export function changeScaledPlayerSize (newSize) {
    scaledPlayerSize = newSize;
}

export function resScalePlayers(reposX, reposY) {
    if (!reposX) {
        reposX = 0;
    }
    if (!reposY) {
    ...
    forAllPlayers((pNum) => {
        player[pNum].size = scaledPlayerSize * resScale();
        player[pNum].x = (player[pNum].x/reposX)*canvas.width;
        player[pNum].y = (player[pNum].y/reposY)*canvas.height;
    })
}
